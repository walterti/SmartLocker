...
class K16V5:
    def __init__(self, i2c_port, i2c_address):
        ...
        self.__bus.write_byte_data(self.__i2c_addr, 0x6, 0x00)

        self.__bus.write_byte_data(self.__i2c_addr, 0x3, 0x00)
        self.__bus.write_byte_data(self.__i2c_addr, 0x7, 0x00)

    def __check(self):
        try:
            self.__bus.read_byte_data(self.__i2c_addr, 0x02)
        except OSErrorr:
            raise OSError(f"XL9535 Board not found at I2C address {self.__i2c_addr:#x}")
